...
def create_dict(course, df_list, keys):
    ...
    if course == COURSE_CATEGORIES[0]:
        for i in range(length):
            dict_final[keys[i]] += len(df_list[i].loc[df_list[i]['Q17'].str.contains(Q17_OPTIONS[4], na = False)])
    elif course == COURSE_CATEGORIES[1]:
        for i in range(length):
            dict_final[keys[i]] += len(df_list[i].loc[df_list[i]['Q17'].str.contains(Q17_OPTIONS[5], na = False)])
    elif course == COURSE_CATEGORIES[2]:
        for i in range(length):
            dict_final[keys[i]] += len(df_list[i].loc[df_list[i]['Q17'].str.contains(Q17_OPTIONS[6], na = False)])
    elif course == COURSE_CATEGORIES[3]:
        for j in range(4):
            for i in range(length):
                dict_final[keys[i]] += len(df_list[i].loc[df_list[i]['Q17'].str.contains(Q17_OPTIONS[j], na = False)])
